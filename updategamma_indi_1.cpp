...
{  
    ...
    for (int i = 0; i < xI; i++) {
        ...
        for (int ss = 0; ss < K1; ss++) { //for each category <=K1
         if (xindicator(i,ss) ==1) {
            /* propose */ 
            gamma_tt[xI*ss+i] = 1-gamma_tt[xI*ss+i];
            ...
            for (int j = 0; j < K1; j++) {
                ...
                log_likeli_nume += gamma_tt[temp]*(log(mik[j]+xa)-log(abI-1))+(1-gamma_tt[temp])*(log(bI-mik[0]-1)-log(abI-1));
                if (gamma_tt[temp] == 0) { lplace0_new +=1;}
                if (gamma_old[j] == 0){ lplace0 += 1;}     
            } 
            if (flag >1 ) {//*
                Iistar = xIstar-1; 
        ...
        //
            ...
            if (log(Rcpp::as<double>(Rcpp::runif(1)) ) <= (log_likeli_nume - log_likeli_deno)) {         
                xmk[ss] = xmk[ss]-gamma_old[ss]+gamma_tt[xI*ss+i];
                gamma_old[ss] = gamma_tt[xI*ss+i];
              ...
              xAg[i] = 0;  
              gamma_tt[xI*ss+i] = gamma_old[ss];     
              gamma_tt[xI*K1+i] = gamma_old[K1]; 
            }
        }  
        }
        int miKstar = xmKstar;
        int Iistar = xIstar;
        int flag =0; int flag_old = 0;
        double log_likeli_deno = 0.; double log_likeli_nume = 0.; int lplace0_new = 0; int lplace0 = 0;
        for (int j = 0; j < K1; j++) {
             temp = xI*j+i;
             mik[j] = xmk[j]-gamma_old[j];
        ...
        if (lplace1_new >1){
              flag_prop = 1;
              gamma_tt[i+xI*K1] = 1-gamma_tt[i+xI*K1];
              if (gamma_tt[xI*K1+i] == 1) {
                   lplace1_new +=1;
                   lplace0 += 1;
               } else {
