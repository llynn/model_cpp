            ...
            for(int k=0; k<K1; k++) {
              temp=xI*k+i;
                if(xd(k,p)==1) {
                    ...
                    if(xgammat[temp]==1) {
                       if (xn_u[temp] >0 && xn_s[temp]>0) {
                           double beta_np = 0.5*xn_s[temp]*xys2_s(temp,p)+0.5*xn_u[temp]*xys2_u(temp,p)+xbeta+pow((muu_p[0]-xybar_u(temp,p)),2)/(2*(xlambda+1/xn_u[temp]))+
                                        pow((xmust[p]-xybar_s(temp,p)),2)/(2*(xlambda+1/xn_s[temp]));
                           double alpha_np = nik/2+xalpha;
                           log1+=-alpha_np*log(beta_np);
                           delF+=(-alpha_np/beta_np)*(muu_p[0]-xybar_u(temp,p))/(xlambda+1/xn_u[temp]);    
                        }else if (xn_u[temp]>0 && xn_s[temp]==0) {
                           double beta_np = 0.5*xn_u[temp]*xys2_u(temp,p)+xbeta+pow((muu_p[0]-xybar_u(temp,p)),2)/(2*(xlambda+1/xn_u[temp]));
                           double alpha_np = nik/2+xalpha;
                           log1 += -alpha_np*log(beta_np);    
                           delF+=(-alpha_np/beta_np)*(muu_p[0]-xybar_u(temp,p))/(xlambda+1/xn_u[temp]);    
                        }                  
                      ...
                      if (nik>0) {
                            double beta_np = 0.5*xn_s[temp]*xys2_s(temp,p)+0.5*xn_u[temp]*xys2_u(temp,p)+xbeta+pow((muu_p[0]-(xn_s[temp]*xybar_s(temp,p)+xn_u[temp]*xybar_u(temp,p))/nik),2)
